...
fn extract_text_from_pdf_bytes(bytes: &[u8]) -> Option<String> {
    ...
    let mut pos = 0usize;
    while pos < bytes.len().saturating_sub(7) {
        let header = &bytes[pos..pos + 7];
        if header == b"stream\n" || header == b"stream\r" {
            let start = pos + 7;
            if let Some(ei) = bytes[start..].windows(9).position(|w| w == b"endstream") {
                let stream_data = &bytes[start..start + ei];
                let dict_start = pos.saturating_sub(300);
                let dict_region = &bytes[dict_start..pos];
                let is_compressed = dict_region
                    .windows(b"/FlateDecode".len())
                    .any(|w| w == b"/FlateDecode");
                if is_compressed {
                    if let Ok(decompressed) = inflate::decompress_to_vec_zlib(stream_data) {
                        decoded.extend_from_slice(&decompressed);
                    }
                } else {
                    decoded.extend_from_slice(stream_data);
                }
                pos = start + ei + 9;
                continue;
            }
    ...
    extract_text_from_pdf_decoded(&decoded)
}
...
mod pdf_tests {
    ...
    #[test]
    fn extract_uncompressed_pdf_text() {
        let pdf = b"%PDF-1.0\n\
1 0 obj<</Type/Catalog>>endobj\n\
2 0 obj<</Length 22>>stream\n\
BT (Hi There) Tj ET\n\
endstream\n\
endobj\n";
        let text = extract_text_from_pdf_bytes(pdf);
        assert!(text.is_some(), "no text extracted");
        assert!(text.unwrap().contains("Hi There"));
    }
}

fn extract_text_from_pdf_decoded(decoded: &[u8]) -> Option<String> {
    let mut result = String::new();
    let mut pos = 0;
    while pos < decoded.len().saturating_sub(2) {
        if decoded[pos..].starts_with(b"BT") {
            pos += 2;
            if let Some(ei) = decoded[pos..].windows(2).position(|w| w == b"ET") {
                let block = &decoded[pos..pos + ei];
                result.push_str(&extract_pdf_strings_from_bytes(block));
                pos += ei + 2;
                continue;
            }
        }
...
fn extract_pdf_strings_from_bytes(block: &[u8]) -> String {
    let mut result = String::new();
    let mut i = 0;
    while i < block.len() {
        if block[i] == b'('
            && let Some(s) = extract_pdf_literal_bytes(&block[i..])
        {
            if !result.is_empty() {
                result.push(' ');
            }
...
fn extract_pdf_literal_bytes(bytes: &[u8]) -> Option<String> {
        ...
        match bytes[i] {
            b'\\' if i + 1 < bytes.len() => {
                ...
                match bytes[i] {
                    ...
                    b'r' => result.push('\r'),
                    b't' => result.push('\t'),
                    b'(' | b')' | b'\\' => result.push(bytes[i] as char),
                    b'0'..=b'7' => {} // octal escapes: skip
                    _ => {}
                }
            }
            b'(' => depth += 1,
